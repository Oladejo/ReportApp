    ...
    public class StaffController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "Id,Title,ReportContent,ReportDate,ReportType")] Report report, HttpPostedFileBase attachedFile)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    var document = new AttachedFile
                    {
                        MimeType = attachedFile.ContentType,
                        FileName = attachedFile.FileName
                    };
                    using (var reader = new BinaryReader(attachedFile.InputStream))
                    {
                        document.Content = reader.ReadBytes(attachedFile.ContentLength);
                    }
                    ...
                    var newReport = new Report
                    {
                        ...
                        AttachedFile = new List<AttachedFile> { document }
                    };
                    _reportRepository.InsertReport(newReport);
                }
                else
                {   //no attached document with the report
                    var newReport = new Report
                    {
                        ProfileId = GetProfile().Id,
                        Title = report.Title,
